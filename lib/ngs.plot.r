...
source('lib/parse.args.r') #Here
source('lib/genedb.r') #Here
source('lib/plotlib.r') #Here
source('lib/coverage.r') #Here
...
default.tbl <- read.delim('lib/database/default.tbl') #Here
dbfile.tbl <- read.delim('lib/database/dbfile.tbl') #Here
...
if(!fi_tag){
    ...
    if(galaxy) {
        ...
        out.plot <- paste(oname, '.avgprof.png', sep='') #Here
    }
    m.pts=5;f.pts=5;mw=5
    png(out.plot, width=1280, height=720, unit="px", res=300, pointsize=3) #Here
